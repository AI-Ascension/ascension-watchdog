...
JSON Schema validates shape and bounds only. Consumers must additionally enforce
the semantic rules in `docs/host-lease-control-contract.md`: grant field
equality, digest and proof domains, gateway/host identity binding, monotonic
renewal, expiry, durable-before-ack ordering, idempotent same-grant retries,
and current boot/fence checks.
...
have valid closed shape because JSON Schema cannot express those invariants.
