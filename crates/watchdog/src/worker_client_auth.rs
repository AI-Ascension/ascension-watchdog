...
//! keeps working:
//!
//! - `identity` owns the [`WorkerPeerIdentity`] model, its trusted
//!   constructors and validation, and the Linux file/seal identity proof.
//! - `credential` owns the credential reference check, the held-descriptor
//!   owner-only read and the bounded zeroizing payload.
//! - `peer` owns live Linux peer validation: controller image capture,
//!   `pidfd`/`/proc/<pid>/exe` inspection, birth-token probing and bounded
//!   image hashing.
//!
//! Every resulting module is below the 1,000-line target.  The functional
//! regression tests stay in this file and still exercise the child modules
//! together.
...
pub use identity::WorkerPeerIdentity;
#[cfg(target_os = "linux")]
pub(crate) use peer::{LinuxPeerSession, authenticate_linux_peer, capture_linux_controller};
...
#[cfg(all(test, target_os = "linux"))]
mod tests {
    use super::identity::LinuxFileIdentity;
