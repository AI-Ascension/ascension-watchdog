...
//! the `worker_client::auth` coordinator so every existing path into the module
//! keeps working.  The parent `worker_client` declares this module with a
//! `#[path]` attribute, so the children are named explicitly instead of relying
//! on directory derivation:
//!
//! - [`models`] owns the worker authentication models: the immutable
//!   `WorkerPeerIdentity` and the Linux file and sealed-image identity records
//!   it retains.
//! - [`credential`] owns credential-reference validation and protected
//!   credential loading, including the held-descriptor Linux open/validate walk
//!   and the bounded read.
//! - [`bootstrap`] owns capture of this controller's own immutable image
//!   identity for worker bootstrap.
//! - [`peer`] owns Linux peer identity validation, the retained peer session,
//!   image hashing and process-start tokens.
//!
//! Every resulting file is below the 1,000-line target, so no exception has to
//! be documented.
...
#[cfg(target_os = "linux")]
#[path = "worker_client_auth/bootstrap.rs"]
mod bootstrap;
...
#[cfg(target_os = "linux")]
pub(crate) use bootstrap::capture_linux_controller;
...
mod tests {
    ...
    use super::peer::{
        LinuxPeerSession, authenticate_linux_peer, hash_file_until, linux_file_identity,
        process_start_token,
    };
