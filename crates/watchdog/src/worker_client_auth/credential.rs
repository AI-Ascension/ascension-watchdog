//! Owner-local worker credential reading and validation.
//!
//! This module owns the credential reference checks, the protected credential
//! object and the deadline-bounded read path, including the held-descriptor
//! ancestor walk, the local-filesystem allowlist and the Windows protected
//! payload adapter.  It opens and validates the exact object before reading it
//! and is called only after worker-peer authentication.

use super::models::MAX_PATH_BYTES;
...
pub(super) fn ensure_deadline(deadline: Instant, phase: &str) -> Result<()> {
