//! Owner-local worker credential reference and held-descriptor reading.
//!
//! A credential is never part of a protocol frame or a durable record.  This
//! module validates the configured reference, walks held non-following
//! directory descriptors to the exact object, validates its owner-only
//! protection and filesystem, and reads a bounded, zeroizing payload.  The
//! held descriptors are retained by [`ProtectedCredential`] so the authority
//! survives a later path replacement.
