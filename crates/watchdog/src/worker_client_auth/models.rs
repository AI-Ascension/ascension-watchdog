//! Worker peer identity models and image-binding proofs.
//!
//! This module owns the immutable identity of the supervised worker process
//! (`WorkerPeerIdentity`), its construction and validation, the Linux image
//! file identity and sealed-image proofs, and the bounded process-start-token
//! and image-digest helpers that bind an identity to an exact running process.
//! Credential reading lives in `credential`; live peer authentication and
//! session retention live in `peer`.
