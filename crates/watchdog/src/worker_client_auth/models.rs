//! Worker authentication models.
//!
//! This module owns the immutable peer identity that binds the supervised
//! worker process to a configured executable, together with the Linux file and
//! sealed-image identity records that identity retains.  The identity is
//! configuration, never a request-controlled value: no constructor accepts a
//! peer's self-reported identity, and credential bytes never pass through it.
...
#[cfg(target_os = "linux")]
use super::peer::{
    hash_file_until, linux_file_identity, process_start_token, require_full_image_seals,
};
