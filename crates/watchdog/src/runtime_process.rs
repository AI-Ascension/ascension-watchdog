...
//! This file stays the `runtime_process` coordinator so every existing
//! `crate::runtime::runtime_process::*` path keeps working.  The supervision
//! facade — the `RuntimeProcessManager` entrypoint, the `RuntimeChild` handle,
//! the observation/stop/launch-error vocabulary and the cleanup-uncertain
//! classification — now lives in the cohesive `facade` child module
//! (`runtime_process/facade.rs`).  The coordinator re-exports every moved name
//! its native dispatch, recovery path and regression tests still call, so no
//! caller changes.
//! This file stays the `runtime_process` coordinator so every existing
//! `crate::runtime::runtime_process::*` path keeps working.  The native
//! backend dispatch and its platform conversions now live in the cohesive
//! `native_backend` child module (`runtime_process/native_backend.rs`); the
//! coordinator imports `NativeBackend`/`NativeChild` back so its facade,
//! recovery path and regression tests are unchanged.
...
#[cfg(all(target_os = "linux", test))]
use crate::platform::{ContainmentId, OwnedProcess, ProcessCreation};
#[cfg(test)]
