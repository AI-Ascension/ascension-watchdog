...
fn owner_published_linux_and_windows_payloads_decode_without_rewriting() {
    for (payload, component_id) in [
        (linux_payload(), "worker.main"),
        (windows_payload(), "worker-main"),
    ] {
        let payload = payload_frame(&payload);
        let decoded = decode_frame(&payload).expect("owner fixture");
