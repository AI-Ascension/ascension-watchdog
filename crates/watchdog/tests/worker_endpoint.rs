...
#[test]
fn bounded_machine_vectors_match_the_owner_resolver() -> Result<(), Box<dyn std::error::Error>> {
    let cases = [
        (
        ...
        (
            ...
            WINDOWS_NAMESPACE,
            NONCE,
            Some(r"\\.\pipe\ascension-worker-12345678-1234-4234-8234-123456789abc"),
        ),
        (EndpointPlatform::Linux, "/", NONCE, None),
        (EndpointPlatform::Linux, "/run/worker", "../escape", None),
    ];
    for (platform, namespace, nonce, expected) in cases {
        let actual = resolve(platform, namespace, nonce);
        match expected {
