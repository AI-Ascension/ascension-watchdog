...
//!
//! The protected credential, payload and service-config readers, the
//! local-path and no-reparse ancestor traversal and the `ProtectedFileAcl`
//! policy enforcement live in the `protected` child module.  This file keeps
//! the Win32 primitives plus the server and client code and re-exports the
//! protected-file entrypoints so every existing import path keeps working.

#![cfg(windows)]

mod protected;

use protected::{open_protected_ancestors, open_worker_image_leaf, validate_local_protected_path};
pub use protected::{
