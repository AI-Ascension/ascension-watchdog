...
//!
//! The shared Win32 transport primitives live in the `win32` child module:
//! the unique-ownership handle and security-descriptor wrappers, bounded
//! frame, timeout and endpoint-name validation, the polling read/write helpers
//! with their pending-I/O classification, the pipe-local-information query and
//! the SID, token, process-image and creation-time identity queries.  This
//! file keeps the server, client and protected-path code plus the entry
//! re-exports so every existing import path keeps working.
...
use win32::{
    FILE_PIPE_CONNECTED_STATE, FILE_PIPE_SERVER_END, OwnedHandle, POLL_INTERVAL,
    SecurityDescriptor, canonicalize_executable, current_user_sid, deadline, is_pending_pipe_error,
    last_error, normalize_path, process_creation_time, query_image_path,
    query_pipe_local_information, read_exact_poll, set_message_nonblocking, sid_string,
    timeout_millis, validate_pipe_name, validate_sid, validate_timeout, validate_worker_pipe_name,
    wide, wide_path, win32_error, write_all_poll,
};
