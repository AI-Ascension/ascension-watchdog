...
//!
//! The administrative and worker pipe clients live in the `client` child
//! module: the admin `connect` and worker `connect_worker` entry points, the
//! mandatory expected-server-executable binding, server account/session/image
//! verification and the bounded request read/write and cancel paths.  This
//! file keeps the Win32 primitives plus the server and protected-path code and
//! re-exports `AdminPipeClient` so every existing import path keeps working.
