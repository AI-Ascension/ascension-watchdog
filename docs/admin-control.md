...
and worker-handoff paths. Restore remains an offline, explicitly rekeyed
operation. Release activation is now owner-integrated: it requires stopped
mode, no owned children or unresolved handoffs, and two protected catalog
inspections around a durable prepared marker and selector commit.
...
approved Unix `owner_uid` to enable inspection and activation. Then
...
selects, activates, launches, or writes an operator receipt. To activate an
inspected digest, use the authenticated admin command
`watchdog release activate --config PATH --release-id ID
--expected-release-digest DIGEST --idempotency-key KEY`. The command records a
`prepared` selector marker before the final protected recheck and atomically
publishes the active digest, previous-release rollback target, audit event, and
replayable receipt. If the process dies after preparation, retrying the exact
request key resumes the final check; a different release is blocked until that
marker is resolved. `watchdog release rollback` uses the same exact-digest
contract with `rollback: true` and can target only the durably recorded
previous release. Restore/rekey clears all release selection. Missing catalog
