...
| S07 | **S07-a** owner-local SQLite WAL/FULL, lock, migrations, integrity/backup/restore; **S07-b** watchdog jobs/attempts/budgets/operator records; **S07-c** gateway/harness stores and stable identity namespaces; **S07-d** bounded archival without silent reset | `crates/watchdog/src/storage.rs`, `storage_admin.rs`, `storage_backup_admin.rs`; `tests/core.rs`, `storage_*`, `admin_backup.rs`, `job_submission*`; fixture SQLite rows in `crates/fault-fixture/src/lib.rs`; gateway `recovery_archive_batch` | `SR`; `SA`; `FF`; `CP`; current-main archive build admission | partial | The gateway component now retains 65 archived duplicate tombstones and rejects conflicting reuse, but harness/gateway owner stores are not integrated with an actual host here. Add cross-owner release tests and a real consumer archival campaign. |
...
| S09 | **S09-a** actual v3 persist-before-send journal and uncertainty; **S09-b** host execution-time fence/witness; **S09-c** restricted broker and >64 archival/retention | Fixture operation/ticket/effect tables and fault tests; gateway `recovery_archive_batch`; watchdog has no gameplay journal | `FF`; `FG`; `RI`; `OR`; `CP`; current-main archive build admission | partial | Gateway component evidence now covers 65 resolved archived tombstones plus duplicate/conflict behavior. The fixture is not the actual v3 path, host broker, or full gateway consumer release; run the real harness→MCP→gateway→mod→host path and archival campaign. |
...
| S12 | **S12-a** Windows SCM/health checker/Job Object/named pipe/session; **S12-b** WSL exact distro/direct invocation/termination; **S12-c** Linux systemd notify/cgroup/protected state; **S12-d** native synthetic execution | `crates/platform-windows/src/{native,admin_pipe}.rs`; Windows tests; `crates/watchdog/src/platform/{linux,linux_launcher,linux_process,wsl}.rs`; `deploy/linux/ascension-watchdog.service`; current native worker smoke | `SI`; `SL`; `docs/platform.md`; `docs/evidence/{windows-p9-integration,native-integration-review,current-main-native-train-20260912}.md`; `CP` | partial | Current-source Linux native worker/process cleanup passed in an active user systemd manager. Installed service, protected-bootstrap, WSL failure, and Windows SCM/session acceptance remain unverified; retain those axes separately. |
...
| INV-13 | Queue/frame/output bounds in admin, platform, and fixture; `receipt_capacity_backpressures_the_sixty_fifth_operation`; gateway `archive_batch_keeps_more_than_sixty_four_duplicate_tombstones`; runtime history backpressure | `FF`; `SA`; current-main archive build admission; `docs/admin-control.md` | partial | Gateway component archival/tombstone semantics beyond 64 are covered, but provider/telemetry/log limits, a real consumer archival campaign, and long-run growth remain open. Execute the full bounded campaign and soak tests. |
...
| FAULT-17 | Gateway `recovery_archive_batch.rs::archive_batch_keeps_more_than_sixty_four_duplicate_tombstones` settles and archives 65 operations, retains every duplicate tombstone, and rejects conflicting reuse; fixture also retains unresolved-work backpressure | partial | The component/archive regression and synchronized current-main build pass, but a real host/consumer archival campaign remains required; do not promote this to live-host evidence. |
...
| FAULT-24 | Accelerated 4,106 paused-loop reconciliations and Collector process replacement exist, but neither is a 24-hour end-to-end soak; `CP` says soak false. The accepted companion prerequisite, pins, single-deployment topology and restart/archive/budget/telemetry-outage matrix are recorded in [`single-deployment-soak-prerequisite-20260917.md`](single-deployment-soak-prerequisite-20260917.md); `deploy/soak/crossrepo-campaign.sh --single-deployment` injects all four kinds and the finalizer requires every kind recovered with monotonic lease epochs; the local two-episode smoke is `unverified` because no host-lease-capable synthetic downstream exists at the pins | unverified | Obtain the host-lease-control-v1 synthetic downstream handoff, confirm two consecutive episodes on one gateway, then run the 24-hour single-deployment soak on a dedicated host with `--single-deployment`; record elapsed duration, bounds, and duplicate-effect evidence. |
...
| FAULT-17 | gateway archive/tombstone component behavior beyond 64 passes | `archive_batch_keeps_more_than_sixty_four_duplicate_tombstones` settles/archives 65 operations, retains duplicate tombstones, and rejects conflicting reuse; a bounded real-consumer campaign remains open |
