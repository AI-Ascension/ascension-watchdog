...
| S05 | **S05-a** ownership architecture; **S05-b** cohesive Rust packages and required docs/schemas/config/deploy/tests; **S05-c** no placeholder/fake-green production paths | Packages `watchdog`, `fault-fixture`, `platform-windows`; `Cargo.*`, `schemas`, `config`, `deploy/linux`, `deploy/windows`, package-level `tests/` suites and docs; package-level tests are the prompt's permitted equivalent to conventional `tests/unit|integration|faults` directories | `docs/architecture.md`; `README`; `docs/evidence/windows-service-wiring.md`; `CP` | partial | Windows packaging sources and non-mutating PowerShell preflight are now present; cross-repo consumer implementation, native service execution, and full acceptance remain open. Run the Windows preflight on the exact head and retain native/install evidence as separate gates. |
