# V31 requirements and evidence audit

Classification: `partial` / `unverified` source-and-test audit at watchdog
`f51d9a33c893af2f49670347644dbb0bb12a60fe`. This audit does not declare an
implementation or release complete.

## Scope and evidence boundary

The audited worktree is an isolated branch from `f51d9a3` (`codex/watchdog-
implementation`). `git diff 8ae0c9bc28811b2b697e062e8d8bf8d0bfb5e1c0..HEAD`
contains only `docs/orchestration/consolidated-integration-checkpoint.json`, so
the source under audit is the source root tested at `8ae0c9b`. The root record
reports:

- `cargo test --locked --offline --workspace --all-targets --all-features` exit
  0, including 49 fixture tests: 8 library, 8 host-lease reference, 13
  recovery, 17 runtime, and 3 schema tests.
- Two delegated Linux cgroup tests were ignored. Windows-native test binaries
  had zero runnable tests on Linux. Windows GNU compilation is not native
  execution evidence.
- No service installation, release activation, game/provider launch, host
  reboot, live recovery, or soak was performed.
- The fixture is a synthetic host/test tool. It is not a gateway, harness, MCP
  server, game mod, watchdog production consumer, or live host.

Out-of-scope later addendum: root reports that a later main revision
`f18650f` adds the Draft 2020-12 host-lease validation cases (9 valid, 1 schema
invalid, 3 semantic-invalid, shape and zero/u53 ACK boundaries) with strict
Clippy passing. That later test count is not retroactively counted in this
`f51d9a3` audit; the root checkpoint's 49-test claim remains the correct base
claim. The same addendum reports a G19 `195f252` gateway review as NO-GO after
reproducing a settled witness followed by old-catalog admission, with G22 repair
active. This reinforces, rather than closes, the production host-lease and
catalog-consumer gaps below.

Evidence abbreviations used below:

| Ref | Evidence record or owned boundary |
| --- | --- |
| `CP` | `docs/orchestration/consolidated-integration-checkpoint.json` at `f51d9a3`; exact root-tested source/evidence boundary and companion heads |
...
| `SI` | `docs/evidence/platform-backup-integration.md`; root integration and 49-test breakdown |
...
1. Implement and integrate the real host-lease consumer path. The reference
   ...
   conformance, including the pending gateway `195f2522338208b32f1b1830b8b1694cb0614a4e`,
   harness `63a53a2`, and mod `4d391ab` work rather than assuming those heads are
   integrated. (`CP`, `OR`, `RW`)
2. Add the harness recovery-sideband implementation: durable canonical
   payload/context, current-authority reconciliation, explicit continuation vs
   reconstruction vs interrupted-unknown, provider accounting, and divergence
   rejection. The harness head has component tests but is not in the watchdog
   release set. (`CP`, `RW`)
...
| S05 | **S05-a** ownership architecture; **S05-b** cohesive Rust packages and required docs/schemas/config/deploy/tests; **S05-c** no placeholder/fake-green production paths | Packages `watchdog`, `fault-fixture`, `platform-windows`; `Cargo.*`, `schemas`, `config`, `deploy/linux`, docs, tests; required `deploy/windows` and conventional `tests/unit|integration|faults` directories are absent | `docs/architecture.md`; `README`; `CP`; required-artifact inventory in this audit | partial | Core artifacts exist, but Windows packaging, cross-repo consumer implementation, and several requested directory forms are absent; production integration remains unsupported. Add/validate the missing deliverables or explicitly document equivalent paths and run them. |
...
| S09 | **S09-a** actual v3 persist-before-send journal and uncertainty; **S09-b** host execution-time fence/witness; **S09-c** restricted broker and >64 archival/retention | Fixture operation/ticket/effect tables and fault tests; watchdog has no gameplay journal; companion gateway/mod heads are outside this source | `FF`; `FG`; `RI`; `OR`; `CP` | partial | Synthetic journal ordering and stale-fence tests pass, but the fixture is not the actual v3 path, host broker, or gateway consumer; 64 is backpressure, not archival. Wire and run the real gateway→MCP→mod→host path and exceed receipt capacity with tombstones. |
| S10 | **S10-a** harness explicit resume; **S10-b** MCP sole-owner bounded reconnect; **S10-c** provider identity/accounting; **S10-d** continuation/reconstruction/interrupted-unknown and replay divergence | No harness implementation in this repository; companion harness `63a53a2` component suites are outside the integrated root source | `CP` reports 81 harness runtime and 10 execution-store tests at the companion head but marks it unintegrated; `RW` says durable payload/context and reconstruction remain open | unverified | Do not count the companion component tests as release proof. Integrate the exact harness sideband/recovery changes, add named resume/reconstruction/divergence/provider fault tests, and rerun cross-repo gates. |
...
| INV-04 | Fixture `response_loss_keeps_effect_witness_and_client_uncertainty`, `malformed_response_does_not_erase_the_durable_receipt`, `crash_after_mutation_retains_witness_without_receipt_or_second_effect` | `FF`; `OR` | partial | Synthetic response loss retains uncertainty, but real gateway transport and host receipt persistence are absent. Run the same cases through actual gateway/MCP/mod consumers. |
| INV-05 | Fixture `crash_after_mutation...`, `reconcile_rejects_a_reference_with_the_wrong_original_context`; runtime replay and instance barrier tests | `FF`; `FG` | partial | No production reconciliation path is proven. Add real operation lookup/reconcile that uses original id/digest and asserts zero second dispatch. |
| INV-06 | Fixture lookup/reconcile returns historical data with `mutation_authorized:false`; `recovery.rs::lookup_rejects_a_reference_with_the_wrong_original_context`; watchdog read/admin separation | `FF`; `SA`; `RI` | partial | Fixture/read API is not the real gateway recovery credential path. Integrate current-authorized historical read and prove old receipt/lease cannot admit mutation. |
| INV-07 | Fixture stale queued fence tests and runtime `http_runtime_rejects_queued_old_lease_after_authority_rotation`; platform process identity/launch proof code | `FF`; `FG`; `docs/platform.md` | partial | No authoritative game host or real mod game-thread boundary is wired. Implement and execute gateway-issued ticket/fence checks immediately before managed mutation. |
...
| INV-09 | Store jobs/attempts/restart records; `tests/core.rs::claims_and_completion_are_atomic_and_idempotent`, `job_submission.rs::submission_replays_original_job_after_completion_and_stop`, admin control stop/replay tests | `SR`; `SA`; `SL` | partial | Watchdog records are tested; provider/episode completion, budgets, and attempt lineage remain outside the integrated source. Add harness provider/job crash and completion tests. |
| INV-10 | Watchdog launch state `prepared/proof_recorded/active/cleaned`; fixture `runtime_releases_instance_barrier_only_after_authoritative_reconciliation` | `FG`; `RI`; `RW` | partial | This does not implement harness in-place continuation vs reconstruction vs interrupted-unknown. Integrate explicit resume/reconstruction policy and divergence-stop tests. |
...
| FAULT-08 | No MCP/provider implementation in this repository; fixture `malformed-response` is host-side only; companion harness head `63a53a2` is unintegrated | unverified | Add MCP death, inference stall, invalid credentials, quota, outage, timeout, cancellation, and conservative billing tests at the harness/provider boundary. |
| FAULT-09 | Watchdog completed-job replay tests (`job_submission*`); no provider/episode consumer | partial | Add harness crash-after-provider-result and crash-after-completed-episode tests with durable result fingerprint and no duplicate inference/job completion. |
| FAULT-10 | Fixture crash/restart and authority rotation tests model a synthetic host; no gateway production path | unverified | Kill/restart the real gateway while host/broker survives; require fresh boot/fence, retain UNKNOWN, and verify no mutation before reconciliation. |
...
| FAULT-18 | No harness reconstruction implementation; `RW` explicitly leaves reconstruction/divergence open; fixture only reconciles synthetic witnesses | unverified | Implement checkpoint/verified-prefix reconstruction with new attempt/incarnation lineage and tests for changed seed/build/catalog and divergent action/state rejection. |
...
`SYNTHETIC_INTEGRATION_VERIFIED = partial` (root-recorded Linux/synthetic
source at `8ae0c9b`), `WINDOWS_SERVICE_VERIFIED = unverified`,
...
`REMOTE_DELIVERY_STATUS = unverified/not completed`. The exact next gate is a
clean cross-repository release-set rebuild after the gateway/harness/mod
