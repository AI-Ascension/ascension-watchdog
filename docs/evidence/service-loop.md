...
The actual daemon thread emits systemd `READY=1` after the first completed
iteration, then `WATCHDOG=1` only for later strictly increasing completed-loop
sequences, and `STOPPING=1` after durable stop and verified child cleanup.
...
- an actual `watchdog daemon` subprocess sends READY then STOPPING to a real Unix
  datagram receiver after a stopped reconciliation; the unit notifier tests
  separately verify the later WATCHDOG heartbeat transition.
...
Targeted tests: 6 passed, 0 failed, 0 ignored. Strict workspace Clippy passed.
